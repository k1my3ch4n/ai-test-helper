...
PR에 자동으로 추가되는 코멘트는 다음과 같은 형식으로 표시됩니다:

```markdown
## AI Test Helper - 테스트 제안

이 PR에 대해 다음 테스트들이 제안되었습니다:

### **[HIGH]** (unit) 사용자 로그인 기능이 올바르게 동작하는지 테스트

<details>
<summary>테스트 코드 예시</summary>

​```typescript
it('유효한 사용자 정보로 로그인 성공해야 한다', async () => {
  const result = await userService.login('test@test.com', 'password');
  expect(result.token).toBeDefined();
});
​```
...
> **참고**: 테스트 코드 예시는 접이식(`<details>`)으로 표시되어 코멘트를 깔끔하게 유지합니다.
...
### 팀 협업 개선
- PR에 자동 테스트 제안 코멘트 (한글로 제공)
- 테스트 코드 예시를 접이식으로 깔끔하게 표시
- 테스트 작성 가이드라인 제공
- 코드 리뷰 효율성 향상
